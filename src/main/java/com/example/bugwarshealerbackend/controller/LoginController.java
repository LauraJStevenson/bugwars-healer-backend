...
public class LoginController {
    ...
    public ResponseEntity<AuthenticatedUser> login (@Valid @RequestBody UserLogin userlogin) {
        ...
        if (loggedInUser == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null); //checks if the user is not found, it returns a 401 Unauthorized status with an empty response body.
        }
